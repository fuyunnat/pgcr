...
void v2_usage(const char *a) {
    fprintf(stderr,
      "PGCR Mirror / MHI2Q V2.2-compatible pixel plane\n\n"
      "Usage: %s --mmi [options]\n       %s --test [options]\n\n"
...
bool v2_parse_options(int argc, char **argv, Options *o) {
    ...
    for(int i=1;i<argc;++i){
        ...
        if(!strcmp(a,"--mmi")){
            if(o->mode!=RUN_MODE_NONE&&o->mode!=RUN_MODE_MMI)return false;
            o->mode=RUN_MODE_MMI;
        }
        else if(!strcmp(a,"--test")){
            if(o->mode!=RUN_MODE_NONE&&o->mode!=RUN_MODE_TEST)return false;
            o->mode=RUN_MODE_TEST;
        }
        else if(!strcmp(a,"--test-seconds")){
            if(!take_int(argc,argv,&i,&o->test_seconds))return false;
        }
        else if(!strcmp(a,"--capture-recover-ms")){
            if(!take_int(argc,argv,&i,&o->capture_recover_ms))return false;
        }
        else if(!strcmp(a,"--fps")){
            if(!take_int(argc,argv,&i,&o->fps))return false;
        }
        else if(!strcmp(a,"--hmi-poll-ms")){
            if(!take_int(argc,argv,&i,&o->hmi_poll_ms))return false;
        }
...
#define PY(name,field) else if(!strcmp(a,name)){ if(!take_int(argc,argv,&i,&o->profiles.field.offset_y))return false; }
        PF("--classic-full-scale",classic_full)
        PX("--classic-full-offset-x",classic_full)
        PY("--classic-full-offset-y",classic_full)
        PF("--classic-small-scale",classic_small)
        PX("--classic-small-offset-x",classic_small)
        PY("--classic-small-offset-y",classic_small)
        PF("--sport-full-scale",sport_full)
        PX("--sport-full-offset-x",sport_full)
        PY("--sport-full-offset-y",sport_full)
        PF("--sport-small-scale",sport_small)
        PX("--sport-small-offset-x",sport_small)
        PY("--sport-small-offset-y",sport_small)
#undef PF
#undef PX
#undef PY

        else if(!strcmp(a,"--classic-full-cover")){
            o->classic_full_cover=true;
        }
        else if(!strcmp(a,"--classic-full-zoom")){
            if(!take_float(argc,argv,&i,&o->classic_full_zoom))return false;
        }
        else if(!strcmp(a,"--classic-full-pan-x")){
            if(!take_float(argc,argv,&i,&o->classic_full_pan_x))return false;
        }
            ...
            set_all_scales(&o->profiles,s);
        }
        else if(!strcmp(a,"--offset-x")){
            int x; if(!take_int(argc,argv,&i,&x))return false;
            set_all_x(&o->profiles,x);
        }
        else if(!strcmp(a,"--offset-y")){
            int y; if(!take_int(argc,argv,&i,&y))return false;
            set_all_y(&o->profiles,y);
        }
        else if(!strcmp(a,"--fullscreen")){
            o->fullscreen=true;
        }
        else if(!strcmp(a,"--verbose")){
            o->verbose=true; o->capture.verbose=true; o->backend.verbose=true;
        }
        else if(!strcmp(a,"--help")||!strcmp(a,"-h")){
            v2_usage(argv[0]); exit(0);
        }
        else {
    ...
    const BaseVideoGeometryProfile *p[]={
        &o->profiles.classic_full,
        &o->profiles.classic_small,
        &o->profiles.sport_full,
        &o->profiles.sport_small
    };
    for(unsigned int i=0;i<sizeof(p)/sizeof(p[0]);++i)
        if(p[i]->scale<=0.0f||p[i]->scale>4.0f)return false;
    ...
    if(o->capture_recover_ms<500||o->capture_recover_ms>60000||
       o->hmi_poll_ms<20||o->hmi_poll_ms>5000||
       o->fps<=0||o->fps>60||o->test_seconds<0)
        return false;
